...
def stage_generate_answers(
    ...
    DB:
        ...
        - Inserts qna_answers rows for each model (one row per model, using that
          model's best sample for this question, plus best/worst flags across models)
        - Inserts one qna_answer_ranking row per question

    Hybrid sampling (per model, per question):
        - NUM_DETERMINISTIC_SAMPLES calls (intended as low-temperature/deterministic)
        - NUM_STOCHASTIC_SAMPLES calls (intended as stochastic/sampling)
        - All samples are scored against context using rank_answers
        - Best and worst sample per model are selected
        - Cross-model ranking uses only the best sample per model
        - Full per-model sample trace is written to JSONL for ORPO-style work
    """
    ...
    with open(out, "w", encoding="utf-8") as fout:
        for item in question_items:
            ...
            for idx, q_text in enumerate(questions, start=1):
                # -------------------------------------------------
                # 1) For each model, generate multiple samples
                # -------------------------------------------------
                per_model_best_answer: Dict[str, str] = {}
                per_model_best_score: Dict[str, float] = {}
                per_model_worst_answer: Dict[str, str] = {}
                per_model_samples: Dict[str, List[Dict[str, Any]]] = {}

                for model_name, model_obj in model_objects.items():
                    # Build a dict of sample_id -> answer_text to reuse rank_answers
                    sample_answers: Dict[str, str] = {}

                    # Deterministic-like passes (e.g. low temp / beams)
                    for i in range(NUM_DETERMINISTIC_SAMPLES):
                        sample_key = f"{model_name}#det{i+1}"
                        try:
                            ans = model_obj.generate_answer(context, q_text)
                        except TypeError:
                            # In case any wrapper still uses a different signature, fall back
                            ans = model_obj.generate_answer(context, q_text)
                        sample_answers[sample_key] = ans

                    # Stochastic-like passes (e.g. sampling)
                    ...
                    # Rank all samples for this model
                    ...
                    if len(ranked_samples) > 1:
                        ...
                        worst_key, worst_ans, worst_score = best_key, best_ans, best_score

                    per_model_best_answer[model_name] = best_ans
                    per_model_best_score[model_name] = float(best_score)
                    per_model_worst_answer[model_name] = worst_ans
                ...
                # If no model produced a valid answer, skip DB + JSONL for this question
                ...
                # -------------------------------------------------
                # 2) Cross-model ranking using each model's best answer
                # -------------------------------------------------
                ...
                rec = {
                    ...
                    # Best answers per model (for backward compatibility)
                    **{f"answer_{m}": a for m, a in per_model_best_answer.items()},
                    # Full per-model sample trace for ORPO / diagnostics
                ...
                #    Each row uses that model's best sample.
                #    Global best/worst are reflected via is_best / is_worst flags.
                # -------------------------------------------------
